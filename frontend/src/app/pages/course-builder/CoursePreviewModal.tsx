          ...
          {course.modules.map((module, mIdx) => (
            <div key={module.id} className="border border-gray-200 rounded-lg overflow-hidden">
              <button
                ...
                className={`w-full px-5 py-4 flex items-center justify-between text-left transition-colors ${expandedModule === module.id ? "bg-indigo-50/30" : "bg-white hover:bg-gray-50"
                  }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${expandedModule === module.id ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500"
                    }`}>
                    {mIdx + 1}
                  </div>
                          ...
                          <div key={q.id || qIdx} className="bg-white p-5 rounded-lg border border-indigo-100/30">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {(q.choices || q.options || []).map((opt: any, oIdx: number) => {
                                ...
                                return (
                                  <div key={oIdx} className={`px-4 py-3 rounded-lg border text-xs font-medium transition-all ${isCorrect

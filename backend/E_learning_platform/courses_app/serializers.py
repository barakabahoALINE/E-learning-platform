...
class SectionSerializer(serializers.ModelSerializer):
    ...
    def get_contents(self, obj):
        ...
        request = self.context.get('request')
        if request and hasattr(request.user, 'role') and request.user.role != 'admin':
            contents = contents.filter(pending_delete=False)
            contents = [c for c in contents if not (c.has_unpublished_changes and not c.draft_title)]
        else:
...
class ModuleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Module
        fields = ["id", "course", "title", "description", "order", "sections","has_unpublished_changes", "pending_delete","quizEnabled", "quiz"]
    ...
    def get_sections(self, obj):
        ...
        request = self.context.get('request')
        if request and hasattr(request.user, 'role') and request.user.role != 'admin':
            sections = sections.filter(pending_delete=False)
            sections = [s for s in sections if not (s.title == "New Section" and s.has_unpublished_changes)]
        else:
    ...
    def get_quiz(self, obj):
        ...
        quiz = Assessment.objects.filter(module=obj, assessment_type="QUIZ").first()
        if quiz:
...
class CourseDetailSerializer(serializers.ModelSerializer):
    ...
    def get_modules(self, obj):
        ...
        request = self.context.get('request')
        if request and hasattr(request.user, 'role') and request.user.role != 'admin':
            modules = modules.filter(pending_delete=False)
            modules = [m for m in modules if not (m.title == "New Module" and m.has_unpublished_changes)]
        else:
    ...
    def get_final_assessment(self, obj):
        from assessments_app.models import Assessment
        from assessments_app.serializers import AssessmentDetailSerializer
        final = Assessment.objects.filter(course=obj, assessment_type="FINAL").first()
        if final:
